...
def mutate(imagine):
    ...
    variation = random.randrange(-5, 5)
...
def fitnessFonction(imagine):
    print(imagine.shape)
    imagine=imagine.reshape(784)
    imagine=imagine.squeeze()
    print(imagine.shape)
    predictionson=model(imagine).numpy()
    risultatopredetto=numpy.argmax(predictionson)
    risultatosecondo=numpy.argmax(predictionson.pop(risultatopredetto))
